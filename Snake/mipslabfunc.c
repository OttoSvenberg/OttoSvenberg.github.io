...
#define U1EP(niklas)		PIC32_R (0x85300 + (niklas << 4)) /* Endpoint control */
...
#define IFS(niklas)		PIC32_R (0x81030+((niklas)<<4)) /* IFS(0..2) - Interrupt Flag Status */
#define IFSCLR(niklas)	PIC32_R (0x81034+((niklas)<<4))
#define IFSSET(niklas)	PIC32_R (0x81038+((niklas)<<4))
#define IFSINV(niklas)	PIC32_R (0x8103C+((niklas)<<4))
#define IEC(niklas)		PIC32_R (0x81060+((niklas)<<4)) /* IEC(0..2) - Interrupt Enable Control */
#define IECCLR(niklas)	PIC32_R (0x81064+((niklas)<<4))
#define IECSET(niklas)	PIC32_R (0x81068+((niklas)<<4))
#define IECINV(niklas)	PIC32_R (0x8106C+((niklas)<<4))
#define IPC(niklas)		PIC32_R (0x81090+((niklas)<<4)) /* IPC(0..12) - Interrupt Priority Control */
#define IPCCLR(niklas)	PIC32_R (0x81094+((niklas)<<4))
#define IPCSET(niklas)	PIC32_R (0x81098+((niklas)<<4))
#define IPCINV(niklas)	PIC32_R (0x8109C+((niklas)<<4))
...
void display_image(int x, const uint8_t *data) {
	...
	for(i = 0; i < 4; i++) {
		...
		for(j = 0; j < 128; j++)
			spi_send_recv(~data[i*128 + j]); //ändrat till 128
	}
}
...
void Fromarrayto_array(void){ //////////////////////////////////////////////////   vi har skrivit!
int shift;                        //delar in array[] i 8-segment och
int tal512 = -1;                  //lägger in de i space[]
int page;
int column;
int pagerad; 
for(page = -1024; page <=2048;){      //börjar från page 0 och incrementar med 1024(nästa page)
  page +=1024;                       
    for(column = -1; column <=126;){     //går igenom de 128 kolumnerna
          column++;                   
          tal512++;                      //incrementar elementet vi är på i space[]
          space[tal512] = 255;           //sätter elementet i space[] till 255
          shift=-1;

      for(pagerad = -128; pagerad<=768;){      //incrementar positionen i pagen med 128
          pagerad += 128;
          shift++;                            
          space[tal512] = space[tal512] - ((array[page+column+pagerad])<<shift);  //elementet space[tal512] - talets värde i array[] och sätter det som = space[tal512]
      }
    }
  }
}
...
int u = -1;
int ax, ay; //äpplets position
void applespawn(){ ////////////////////////////////////////////////////   vi har skrivit!
  u++;
  if(u > 1000){            //ser till så att vi inte får slut på äpplen
    u = 0;
  }
  for(u; u < 4096; u++){
    if(array[applearray[u]*15] == 0){        //hittar en ledig plats på displayen och spawnar ett äpple
      array[applearray[u]*15] = 1;
      ay = (applearray[u]*15) / 128;         // bestämmer x och y för äpplet
      ax = (applearray[u]*15) % 128; 
    break;
    }
  }
}
...
int x = 0;
int y = 0;
void startprogram(void){////////////////////////////////////////////////////    vi har skrivit!
array[0] = 1;           // startar snaken vid x,y(0,0)
}


int direction;
int getdirection(){
  if(direction == 1){} //om det är motsatt riktning så gör vi ingenting
    else{
    if((getbtns() & 0x4) != 0){ //1000  //vänster     
    direction = 4;
    }  
  }
  if(direction == 2){}
  else{
    if((getbtns() & 0x2) != 0){//0100 //upp
    direction = 3;
  }
  }
  if(direction == 3){}
  else{
      if((getbtns() & 0x1) != 0){//0010  //ner
      direction = 2;
  }
  }
  if(direction == 4){}
  else{  ////////////////////////////////////////////////////    vi har skrivit!
   if((getbutton1() & 0x1) != 0){//0001  //höger 
    direction = 1;    
  }
  }
...
void dead(){/////////////////////////////////////////////////////////////////////   vi har skrivit!
  int j;
  for(j =0 ; j< 4096; j++){
    array[j] = 0;
  }                                  //visar antal äpplen ätna om vi dör
  while(1==1){
    display_update();
    display_string( 0, itoaconv( appleeaten ) );
  }
}


int snakelength = 1;  //startlängden
int q = 0;
uint8_t larray[];

void movesnake(){              /////////////////////////////////////////////////  vi har skrivit!
int j;
if(x < 0 | x > 127 | y < 0 | y > 31 ){        //kollar om snaken går utanför spelplanen
  dead();
}
else{
int newdirect = getdirection();              
int newx = x; //nya x värdet efter vi rört oss
int newy = y; //nya x värdet efter vi rört oss
int L;
int i;


if(newdirect == 1){
  if(array[1+x+(y*128)] == 1 && x+1 != ax){  //kollar om nya positionen är snaken isf dör vi      
    dead();
  }
  if(x+1 == ax && y == ay){ //om positionen är ett äpple
    array[1+x+(y*128)] = 0; //sätt positionen till 0 så att äpplet försvinner
    appleeaten++;  //lägg till score
    applespawn(); //lägg till äpplet
    snakelength++;    // gör ormen större
  }
array[1+x+(y*128)]= 1;    //flyttar snakens huvud 
x++;    //ökar x med 1 så vi fortsätter till höger
newx = x; //vi får en ny position för ormen
larray[q]=4;  //motsatt riktning
L=q; // motsatt riktning och sedan sätter L-- i slutet för att kolla förra riktningen 
}
if (newdirect == 2){
  if(array[x+((y+1)*128)] == 1 && y+1 != ay){
  dead();
  }
   if(x == ax && y+1 == ay){
    array[x+((y+1)*128)] = 0;
...
q++; //kommer till nästa element i larray som kollar förra movet

for(i = 0; i<= snakelength;){
i++;


if(larray[L]==1){
newx++;                               //går igenom hela snaken och hittar den sista pixeln och tar bort den
if(i==snakelength){
array[newx+(128*newy)]=0;
break;
}
