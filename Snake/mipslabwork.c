...
volatile int *trisE; // volatile pointer trisE
volatile int *portE; // poiter portE
...
void labinit( void )
{
  ...
  *portE = 0x00000000;
  TRISD = TRISD | 0xfe0; //0000 1111 1110 0000
  TRISF = TRISF | 0xe;  // 1110

  T2CON = 0x0; //stop timer and clear control register
  T2CONSET = 0x70;//set prescale to 1256. Timer referensheet p.9
  TMR2 = 0x0; //clear time register
  PR2 = ((80000000 / 256) /20);//load Period register
  T2CONSET = 0x8000;//start timer

  
  //family data sheet 14, 51
  IFSCLR(0) = 0x100;
  IPCSET(2) = 0x1f; //T2ip is at bit 4-2 for priority and 1-0 for sub priority set those to one with 0001 1111 = 0x1f Lecture 6 set higher priority
  IECSET(0) = 0x100;//T2if is at bit 8  1 0000 0000 in order to enable IEC
  ...
  enable_interrupt();//calls labwork.s function to enable interrupt globally
...
/////Because the caller has to save the temporary registers. You only need to save temporary registers.

//5.Which device-register (or registers), and which processor-register (or registers) must be
//written to enable interrupts from the timer? Describe the functions of the relevant registers.

/////IEC = enable interrupt  and IPC  = interrupt priority control
